// Shared example program catalogue — the single source of truth for both apps/web's Examples
// dropdown and the VS Code extension's "TT: Insert Example" command, so the two never drift.
// English label/description text here doubles as the i18n fallback in apps/web (looked up by a
// slug derived from the label); the VS Code extension has no localization, so it uses this text
// directly.
